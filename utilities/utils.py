...
def cp_bias_from_trainset_to_valset(train_dset, eval_dset):
    """ copy the computed question_type_to_prob_array from train_dset to eval_dset
    """
    for ex in eval_dset.dataset.entries:
        q_type = ex["question_type"]
        ex["bias"] = train_dset.dataset.question_type_to_prob_array[q_type]
