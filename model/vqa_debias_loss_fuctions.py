# this file is copied from chrisc36'repository: bottom-up-attention-vqa
...
class DebiasLossFn(nn.Module):
    ...
    def forward(self, hidden, logits, bias, labels, opt_mask):
        """
...
class LearnedMixin(DebiasLossFn):
    def __init__(self, hidden_size=1024, w=0.36, smooth=True, smooth_init=-1, constant_smooth=0.0):
        """
        ...
        self.smooth_init = smooth_init
        self.constant_smooth = constant_smooth
        self.bias_lin = torch.nn.Linear(hidden_size, 1)
        self.smooth = smooth
        if self.smooth:
            self.smooth_param = torch.nn.Parameter(
              torch.from_numpy(np.full((1,), smooth_init, dtype=np.float32)))
    ...
    def forward(self, hidden, logits, bias, labels, opt_mask):
        ...
        entropy = -(torch.exp(bias_logprob) * bias_logprob).sum(2)

        if opt_mask is not None:
            eps = 1e-7
            loss = ((loss * opt_mask).sum()+ eps)/ (opt_mask.sum()+ eps)
            entropy = ((entropy.mean(1) * opt_mask).sum() + eps)/ (opt_mask.sum()+ eps)
        else:
            loss = loss.mean(0)
